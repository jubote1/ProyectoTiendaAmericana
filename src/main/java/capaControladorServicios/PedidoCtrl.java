	...
	public void run() {
		...
		{   
			for(int i = 0; i < JSONPedidosNoti.size(); i++)
			{
				...
				// En este punto vamos a realizar la l�gica de implementaci�n
				...
				if(clientePed.getPoliticaDatos().equals(new String("S")))
				{
					...
					//verificamos la existencia de correo electr�nico para de esta manera realizar el env�o de este
					if(clientePed.getEmail() != null)
					{
						if(clientePed.getEmail().length()> 0)
						{
							...
							String mensajeCuerpoCorreo = "Querido Cliente!!! Pizza Americana te informa que el domiciliario ya est� en ruta para entregarte tu pedido # " + idPedido + ". Disfrutala"
...
//		CrudFirebase.eliminarPedido(38321, 13);
	...
	public String InsertarPedidoPixel(String datos)
	{
		// Debemos de procesar todo datos para extraer todo los datos requeridos
		 ...
		 try {
			 ...
			 //Controlamos excepci�n en caso de que no la encuentre
			 try
			 {
			 ...
			 //Capturamos la informaci�n de si se debe reintegrar o no el descuento
			 try
			 {
			 ...
			 //Capturamos la informaci�n de programaci�n del pedido
			 try
			 {
			 ...
			 //Incluimos la modificaci�n para tomar los datos de plataforma de pedidos
			 try
			 {
			 ...
			 //Realizamos la validaci�n de que POS para terminar el pedido.
			 if(POS == 2)
			 {
				 //Bajo esta condici�n estamos en el sistema Pixel POS
	...
	/**
	 * M�todo en la capa controladora que se encarga de la inserci�n del pedido tomado desde el contac center
	 ...
	 * @param cliente Objeto tipo cliente que encapsula toda la informaci�n del cliente
	 * @param indicadorAct --
	 * @param dsnTienda --
	 * @param envioPixel Arreglo con toda la informaci�n del detalle del pedido
	 ...
	 * @param usuario usuario que est� tomando el pedido
	...
	public static RespuestaPedidoPixel confirmarPedidoPOSPMTienda(int idpedido,double valorformapago, double valortotal, Cliente cliente, boolean indicadorAct, String dsnTienda, ArrayList<DetallePedidoPixel> envioPixel, int idformapagotienda, int idTienda , String usuario, int tiempoPedido, double descuento, String motivoDescuento, String reintegro, String origen, String programado, String horaProgramado, int idTipoPed, String plataforma, String idPedidoAlt, String domicilioTercerizado, String empresaTercerizada)
	{
		...
		//Validaremos si el sistema est� aperturado en la tienda
		...
		//Iremos a separar la l�gica de lo nuevo y de lo pasado
		...
		//conformamos el campo direcci�n con base en la nomenclatura definida
		...
		//Antes de realizar la actualizaci�n o modificaci�n del cliente vamos a revisar su estado del memcode
		...
		//Creamos el objeto cliente con todos los par�metros
		if(cliente.getMemcode() == 0)
		{
			//Realizamos la creaci�n del cliente capturando todos los valores enviados como par�metros
			...
			//Realizamos la inserci�n del cliente
		...
		{
			//Creamos objeto para la actualizaci�n del cliente
			...
			//Obtenemos el idClienteTienda del valor que ya viene en los par�metros dado que ya este existe.
			idClienteTienda = cliente.getMemcode();
			//Realizamos la actualizaci�n del cliente si viene al caso
		...
		// Esta variable hace alusi�n a como viene el idDetalleMaster del sistema de contact center que obviamente ser�
		//diferente al n�mero que queda en el sistema tienda
		int idDetalleMaster = 0;
		// Esta variabla hace alusi�n al idDetallePedido que ya le queda al idDetalleMaster luego de la inserci�n
		// en el sistema de tienda.
		int idDetalleMasterDef = 0;
		// La siguiente variable hace alusi�n a la idDetalleMaestro en el detalle hijo
		...
		String strPrecio = "";
		//Antes de iniciar la inserci�n del detalle del pedido revisamos si hay que insertar el m�gnetico
		if(creaCliente)
		{
		...
		for(int i = 0; i  < envioPixel.size(); i++)
		{
			...
			//Validamos si el precio devuelto es -1 es porque el precio deber� ser el que viene en la integracion
			if(precio == -1 )
			{
			...
			if(esMaster)
			{
				...
				//Guardamos el y el idDetalle generado en la inserci�n
			...
			}else if(idProducto != 2002)
			{
				...
				if(idDetalleHijoMaster == idDetalleMaster)
				{
					...
					}else //Sino se cumple esta condici�n es porque es un modificador por lo tanto hay que devolverse a ver el modificador
					{
						//Tenemos qeu devolvernos en el arreglo para saber cual es el padre del modificador es decir
						//el modificador a quien est� relacionado
		...
		//Definimos la variable que tendr� el tipo de pedido a insertar
		int idTipoPedido = 0;
		...
		//NOS TOCA AFINAR LOS PAR�METROS PARA INSERTAR CORRECTAMENTE LA FORMA DE PAGO
		...
		//Revisamos si dentro de la informaci�n proveniente del contact center hay descuento
		if(descuento > 0)
		{
			if(reintegro == null)
			{
			...
			//Realizamos la inserci�n del descuento en base de datos
		...
		if(idPedidoTienda != 0)
		{
			...
			//Una vez realizamos la inserci�n del pedido, insertamos el pedido en la tabla para control de duplicado
	...
	public ArrayList<TiempoPedido> calcularTiemposPedidos(String fecha)
	{
		...
		//el ciclo tendr� vida mientras existe estados de historia de pedido a procesar
		...
		while(hisEstadosPedidos.size() > 0)
		{
			...
			//Extraemos el idPedido que vamos a tratar para esta iteraci�n
			...
			//En este punto ya podemos realizar validaci�n si el estado es final o no y de acuerdo a esto tomar ciertas
			//determinaciones.
	...
	//Realizamos desarrollo en capa controlador de lo que atender� la necesidad de consulta de pedidos 
	//Sobre este m�todo agregaremos un nuevo campo qeu es comentario sobre el cual se tendr� en cuenta si el pedido tiene o no comentarios
	...
	/**
	 * M�todo que tendr� como objetivo avanzar de estado un pedido que fue marcado como entregado por un domiciliario
	 * desde la app de log�stica
	...
	//M�todo que se encargar� de dar llegada a los domiciliarios desde el sistema TABLET
		...
		{
			//Creamos el JSONArray con el JSON que se recibe como par�metro
			...
			{
				System.out.println("Error parseando el JSON recibido por aplicaci�n Tablet " + e.toString());
			}
			//Se traen el listado de pedidos 
			ArrayList pedidos = new ArrayList();
			capaControladorPOS.PedidoCtrl pedidoCtrlTienda = new capaControladorPOS.PedidoCtrl(false);
			...
			for(int i = 0; i < JSONPedidos.size(); i++)
			{
				...
				int idPedido = Integer.parseInt((String)pedido.get("idpedido"));
				//Realizamos la salida del domicilio en espec�fico
			...
			//Realizamos la creaci�n del hilo para realizar la notificacion del despacho
...
//			hiloDespachos.start();
		...
		/**
		 * M�todo para dar entregado un pedido desde las plataformas de domicilios
		 * @param idUsuario
		 * @param usuario
		 * @param idPedido
		 * @return
		...
		public String darSalidaDomicilioPlataforma(String idUsuario, String usuario, int idPedido)
		{
			...
			estEnRutaDom = valNum;		
			//Realizamos la actualizaci�n del pedido
		...
		//M�todo que se encargar� de devolver de estado domicilios que un domiciliario pudo seleccionar por error
		public String devolverEstadoDomicilio(String idUsuario, String usuario, String JSONidPedido)
		{
			//Creamos el JSONArray con el JSON que se recibe como par�metro con el o los pedidos en formato JSON
			JSONArray JSONPedidos = new JSONArray();
			try
			{
				JSONParser parser = new JSONParser();
